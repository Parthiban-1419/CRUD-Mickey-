...
// org admin previlaged
if(!user.isOrgAdmin()){
	out.println("User is not org admin");//No I18N
	return;
}

//client details here
String restricted_type= request.getParameter("restrictedType");
String clientType = request.getParameter("clientType");
String serviceUrl=request.getParameter("serviceurl");
String serviceName = request.getParameter("servicename");

if(!Util.isValid(restricted_type)){
	out.println("Mandatory parameter missing : restrictedType");//No I18N
	return;
} else if(!Util.isValid(clientType)){
	out.println("Mandatory parameter missing : clientType");//No I18N
	return;
}
// serviceUrl = serviceUrl.indexOf("://")!= -1 ? serviceUrl : serviceUrl.startsWith("/") ? serviceUrl : "/"+serviceUrl;
// Service service = Util.isValid(serviceName) ? Util.SERVICEAPI.getService(serviceName) : null;
...
function addAPIClient(f){
	...
	var client_type = '<%=IAMEncoder.encodeJavaScript(clientType)%>';
	var restrictedType = '<%=IAMEncoder.encodeJavaScript(restricted_type)%>';
	var client_redirect_uri = f.client_redirect_uri.value.trim();
	var client_js_domain = '';
	if(client_type == 'JS'){
		client_js_domain = f.client_js_domain.value.trim();
	}
	if(client_name === '') {
		showAPIClientError(f.client_name, "<%=Util.getI18NMsg(request, "IAM.DEVELOPERCONSOLE.ENTER.CLIENTNAME")%>");
		return false;
	} else if(!checkForSpecialChar(client_name)){
		showAPIClientError(f.client_name, "<%=Util.getI18NMsg(request, "IAM.DEVELOPERCONSOLE.ENTER.CLIENTNAME")%>");
		return false;
		...
		return false;
	} else if (client_type === 'JS' && client_js_domain === ''){//No I18N
		showAPIClientError(f.client_js_domain, "<%=Util.getI18NMsg(request, "IAM.DEVELOPERCONSOLE.ENTER.JS.DOMAIN")%>");
		return false;
	}
	
	if((client_type === 'JS' || client_type === 'WEB') && !urlPattern(client_redirect_uri)) {
		showAPIClientError(f.client_redirect_uri, "<%=Util.getI18NMsg(request, "IAM.DEVELOPERCONSOLE.ENTER.CLIENTREDIRECTURL")%>");
		return false;
	}
	if (client_type === 'JS' && !urlPattern(client_js_domain)){//No I18N
		showAPIClientError(f.client_js_domain, "<%=Util.getI18NMsg(request, "IAM.DEVELOPERCONSOLE.ENTER.JS.DOMAIN")%>");
		return false;
	}
	
	var params = "client_name=" + euc(client_name) + "&client_type=" + euc(client_type) + "&client_redirect_uri=" + euc(client_redirect_uri) + "&restricted_type=" + euc(restrictedType) + "&client_domain=" + euc(client_domain) + "&" + csrfParam; //No I18N
	if(client_type === "JS") {
	...
	var resp = getPlainResponse("/oauth/v2/create/restrictedclient", params); //No I18N
	resp = resp.trim();
	var path = getServiceUri();
	if(resp.indexOf("success_") !== -1) {
		clientzid = resp.split("_")[1];
		var clientId = resp.split("_")[2];
		var clientSecret = resp.split("_")[3];
		...
		de('add_client_container_response_1').style.display = '';
		window.setTimeout(function(){window.location.href=redirectToServiceUrl()},10000);
...
function mobileurlPattern(url){
	 var pattern = new RegExp('[a-zA-Z0-9\-_]\:\/\/[-.\w]*(\/?)([a-zA-Z0-9\-\.\?\,\:\'\/\\\+=&amp;%\$#_@]*)?$');	
	 if(!pattern.test(url)) {
		return false; 
	 }
	 return true;
...
<div class="add_client_main_div" id="add_client_form_container">
	...
	<div class="add_client_formdiv">
		<form name="add_api_client" method="POST" onsubmit="return addAPIClient(this)">
			...
			<% if(clientType.equalsIgnoreCase("JS")){%>
			<div class="add_client_form_field_main_div">
